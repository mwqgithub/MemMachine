...
DEFAULT_PG_USER = "memmachine"
DEFAULT_PG_PASSWORD = "memmachine_password"
...
class ConfigurationWizard:
    def __init__(self, args: ConfigurationWizardArgs, logger: logging.Logger):
        ...
        self.neo4j_host: str = args.neo4j_host if self.neo4j_provided else ""
        self.neo4j_port: int = args.neo4j_port if self.neo4j_provided else 0
        self.neo4j_user: str = args.neo4j_user if self.neo4j_provided else ""
    ...
    def ask_pg_and_neo4j_configs(self):
        ...
        if not self.neo4j_provided:
            self.neo4j_host = (
                input(f"Enter Neo4j host [{DEFAULT_NEO4J_HOST}]: ").strip()
                or DEFAULT_NEO4J_HOST
            )
            self.neo4j_port = self.parse_port(
                input(f"Enter Neo4j port [{DEFAULT_NEO4J_PORT}]: ").strip()
                or DEFAULT_NEO4J_PORT
            )
            self.neo4j_user = (
                input(f"Enter Neo4j user [{DEFAULT_NEO4J_USER}]: ").strip()
    ...
    def create_configuration(self):
        ...
        for storage in config.get("storage", {}).values():
            if storage.get("vendor_name") == "neo4j":
                ...
                self.logger.warning(
                    f"Unknown storage vendor: {storage.get('vendor_name')}"
                )
